 ...
 * somente aqueles elementos que estão na área inferior
 ...
 * Seguem os 144 valores de ponto flutuante de dupla precisão
 * (double) que compõem a matriz.
 * Saída
 * Imprima o resultado solicitado (a soma ou média), com 1
...
//                matriz[linha][coluna] = 1; // Para testes
        ...
        for (int linha = 7; linha <= 11; linha++) {
            for (int coluna=12-linha ; coluna<=5 ; coluna++) {
        ...
        for (int linha = 7; linha<=11; linha++) {
            for (int coluna = 6; coluna<=linha-1; coluna++) {
                soma += matriz[linha][coluna];
                contElementosSoma++;
            }
        }
